...
class AuthService {
    static async userRegister(name, nik, phone_number) {
        ...
        return prisma.$transaction(async (tx) => {
            ...
            await tx.profiles.create({ data: { nik, user_id: data.id } })

            await tx.roles.create({
                data: { user_id: data.id, name: 'partner' },
            })
        })
    }
    ...
    static async userLogin(phone_number) {
        ...
        if (foundUser.roles.name != 'partner') {
            throw new ErrHandler(403, 'You are not partner role.')
        }
